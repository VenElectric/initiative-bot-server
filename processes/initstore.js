...
function delete_init(session_id,id){
    let initRef = sessionRef.doc(session_id)
    initRef.collection('initiative').doc(id).delete().then(()=>{
        console.log('Success?')
      })
      .catch((error)=>{
        console.log(error)
...
async function remove_all_init(session_id){
    let snapshot = await sessionRef.doc(session_id).collection('initiative').get()
    let batch = db.batch();
    snapshot.docs.forEach((doc)=>{
        batch.delete(doc.ref);
    })
    sessionRef.doc(session_id).set({sorted:false,on_deck:0},{merge:true}).then(()=>{
        console.log('success')
    }).catch((error) => {
...
module.exports = {add_init,write_all,get_all,update_init,delete_init,add_spell,delete_spell,update_spell,remove_all_init,remove_all_spells}